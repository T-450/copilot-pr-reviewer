...
/**
 * Recursively extract a plain text string from an unknown SDK response value.
 *
 * The SDK's `sendAndWait` return type is `unknown`.  In practice the value is
 * one of: a string, an array of message objects, or a nested object with a
 * well-known text key.  This function normalises all of those shapes into a
 * single string so callers don't need to handle each case individually.
 */
...
/**
 * Type guard that checks whether a thread is an actionable reply candidate:
 * status must be active (1) and there must be an unanswered user follow-up.
 */
...
/**
 * Sort candidate threads so the oldest unanswered follow-up is processed first,
 * preventing newer follow-ups from starving older ones across runs.
 */
